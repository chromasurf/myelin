...
static void count_warnings(MylLogLevel level, const char *message, void *user_data)
{
    ...
    if (level == MYL_LOG_WARNING) {
        warning_count++;
        if (getenv("MYL_SHOW_WARNINGS")) fprintf(stderr, "WARN: %s\n", message);
    }
}

static MylManifest *parse(const char *json)
{
    warning_count = 0;
    return myl_manifest_parse(json, strlen(json), "unit", "/tmp/unit");
}
...
static void test_basic_parse(void)
{
    ...
    if (m->n_scripts == 1) {
        const MylContentScript *s = &m->scripts[0];
        ...
        CHECK(s->run_at == MYL_RUN_AT_DOCUMENT_END);
        CHECK(!s->all_frames);
        CHECK(myl_content_script_matches(s, "http://localhost:4000/dashboard"));
        CHECK(!myl_content_script_matches(s, "https://example.com/"));
    }
...
static void test_defaults(void)
{
    ...
    MylManifest *m = parse("{\"content_scripts\":[{"
    ...
    CHECK(!m->enabled);
    CHECK(m->scripts[0].run_at == MYL_RUN_AT_DOCUMENT_END);
...
static void test_run_at_variants(void)
{
    ...
    struct {
        ...
        { "document_start", MYL_RUN_AT_DOCUMENT_START, 0 },
        { "document_end", MYL_RUN_AT_DOCUMENT_END, 0 },
        { "document_idle", MYL_RUN_AT_DOCUMENT_IDLE, 0 },
        /* An unknown value warns and falls back rather than failing the load. */
        { "whenever", MYL_RUN_AT_DOCUMENT_END, 1 },
    };
...
static void test_exclude_matches(void)
{
    ...
    CHECK(myl_content_script_matches(&m->scripts[0], "http://localhost/dashboard"));
    CHECK(!myl_content_script_matches(&m->scripts[0], "http://localhost/admin/users"));
...
static void test_unknown_keys_warn_but_load(void)
{
    ...
    CHECK(m->n_scripts == 1);
    CHECK(myl_content_script_matches(&m->scripts[0], "http://localhost/"));
...
static void test_config_slice(void)
{
    ...
    {
        MylManifest *m = parse("{\"config\": {\"a\": 1, \"b\": {\"c\": [true, null]}},"
    ...
    {
        MylManifest *m = parse("{\"content_scripts\":[{\"matches\":[\"<all_urls>\"],"
                               "\"js\":[\"a.js\"]}]}");
    ...
    {
        MylManifest *m = parse("{\"config\": 5,\"content_scripts\":[{"
                               "\"matches\":[\"<all_urls>\"],\"js\":[\"a.js\"]}]}");
    ...
    {
        MylManifest *m = parse("{\"config\": {\"a\": \"x\ty\"},\"content_scripts\":[{"
                               "\"matches\":[\"<all_urls>\"],\"js\":[\"a.js\"]}]}");
    ...
    {
        MylManifest *m = parse("{\"config\": {\"a\": foo},\"content_scripts\":[{"
                               "\"matches\":[\"<all_urls>\"],\"js\":[\"a.js\"]}]}");
    ...
    {
        MylManifest *m = parse("{\"config\": {\"a\": -0.5e+3, \"b\": null, \"c\": false},"
                               "\"content_scripts\":[{\"matches\":[\"<all_urls>\"],"
                               "\"js\":[\"a.js\"]}]}");
...
static void test_relative_paths(void)
{
    ...
    CHECK(myl_relative_path_ok("branding.js"));
    CHECK(myl_relative_path_ok("build/branding.js"));
    CHECK(myl_relative_path_ok("build/nested/branding.js"));

    /* Everything that could leave the script's directory, or is simply not a
     * file name. */
    CHECK(!myl_relative_path_ok("../branding.js"));
    CHECK(!myl_relative_path_ok("a/../../etc/passwd"));
    CHECK(!myl_relative_path_ok("/etc/passwd"));
    CHECK(!myl_relative_path_ok("a//branding.js"));
    CHECK(!myl_relative_path_ok("build/"));
    CHECK(!myl_relative_path_ok("."));
    CHECK(!myl_relative_path_ok(".."));
    CHECK(!myl_relative_path_ok("a/./b.js"));
    CHECK(!myl_relative_path_ok("build\\branding.js"));
    CHECK(!myl_relative_path_ok(""));
    CHECK(!myl_relative_path_ok(NULL));
...
{
    char **parts = myl_split_search_path("/a:/b:/c");
    ...
    parts = myl_split_search_path("/a::/b:");
    CHECK(parts != NULL);
    if (parts) {
        CHECK_STREQ(parts[0], "/a");
        CHECK_STREQ(parts[1], "/b");
    ...
    CHECK(myl_split_search_path("") == NULL);
    CHECK(myl_split_search_path(NULL) == NULL);
}
...
static void test_search_path_override(void)
{
    ...
    list = myl_manifest_load_path(path, 2);
    CHECK(list != NULL);
    if (list) {
...
static void test_example_manifests(void)
{
    ...
    for (size_t i = 0; i < list->n_items; i++) {
        const MylManifest *m = &list->items[i];
        ...
        for (size_t s = 0; s < m->n_scripts; s++) {
            const MylContentScript *cs = &m->scripts[s];
            ...
            CHECK(cs->run_at == MYL_RUN_AT_DOCUMENT_END);
            CHECK(myl_content_script_matches(cs, "http://localhost:4000/"));
        }
    }
